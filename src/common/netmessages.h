...
inline bool(*CLC_CmdKeyValues_ReadFromBuffer)(Base_CmdKeyValues* thisptr, bf_read* buffer);
inline bool(*CLC_CmdKeyValues_WriteToBuffer)(Base_CmdKeyValues* thisptr, bf_write* buffer);
inline void* g_pCLC_CmdKeyValues_VFTable = nullptr;
inline bool(*SVC_CmdKeyValues_ReadFromBuffer)(Base_CmdKeyValues* thisptr, bf_read* buffer);
inline bool(*SVC_CmdKeyValues_WriteToBuffer)(Base_CmdKeyValues* thisptr, bf_write* buffer);
inline void* g_pSVC_CmdKeyValues_VFTable = nullptr;
...
class V_NetMessages : public IDetour
{
	virtual void GetAdr(void) const
	{
		...
		LogConAdr("CLC_CmdKeyValues::`vftable'", g_pCLC_CmdKeyValues_VFTable);
		LogConAdr("SVC_CmdKeyValues::`vftable'", g_pSVC_CmdKeyValues_VFTable);
	...
	virtual void GetCon(void) const
	{
		...
		g_pCLC_CmdKeyValues_VFTable = g_GameDll.GetVirtualMethodTable(".?AVCLC_CmdKeyValues@@");
		g_pSVC_CmdKeyValues_VFTable = g_GameDll.GetVirtualMethodTable(".?AVSVC_CmdKeyValues@@");
	}
...
//-------------------------------------------------------------------------
inline void* g_pSVC_DurangoVoiceData_VFTable = nullptr;
...
public:
	...
	{
		void** pVFTable = reinterpret_cast<void**>(this);
		*pVFTable = g_pSVC_DurangoVoiceData_VFTable;
...
// svc_Sounds (msg id 17). One entity-attached sound per message; no batching.
class SVC_Sounds : public CNetMessage
{
public:
	SVC_Sounds() = default;
	SVC_Sounds(int tick, uint64_t soundHash, int entityIndex) :
		m_ticks(tick), m_soundHash(soundHash), m_entityIndex(entityIndex)
	{
		m_nGroup = NetMessageGroup::NoReplay;
		m_bReliable = false; // transient one-shot event, matches EmitSoundOnEntity semantics
	}

	virtual bool			ReadFromBuffer(bf_read* buffer);
	virtual bool			WriteToBuffer(bf_write* buffer);

	virtual bool			Process(void) { return true; } // dedi only sends, never receives its own broadcast

	virtual int				GetType(void) const { return svc_Sounds; }
	virtual const char*		GetName(void) const { return "svc_Sounds"; }

	virtual const char*		ToString(void) const
	{
		static char szBuf[128];
		V_snprintf(szBuf, sizeof(szBuf), "%s: ent %d, hash %016llX", GetName(), m_entityIndex, m_soundHash);
		return szBuf;
	}

	virtual size_t			GetSize(void) const { return sizeof(SVC_Sounds); }

	int      m_ticks;
	uint64_t m_soundHash;
	int      m_entityIndex;
};
...
class V_NetMessages : public IDetour
{
	virtual void GetAdr(void) const
	{
		...
		LogConAdr("SVC_ServerTick::`vftable'", g_pSVC_ServerTick_VFTable);
		LogConAdr("SVC_VoiceData::`vftable'", g_pSVC_VoiceData_VFTable);
		LogConAdr("SVC_DurangoVoiceData::`vftable'", g_pSVC_DurangoVoiceData_VFTable);
		LogConAdr("SVC_PlaylistOverrides::`vftable'", g_pSVC_PlaylistOverrides_VFTable);
		LogConAdr("CLC_ClientTick::`vftable'", g_pCLC_ClientTick_VFTable);
		LogConAdr("CLC_SetPlaylistVarOverride::`vftable'", g_pCLC_SetPlaylistVarOverride_VFTable);
		LogConAdr("Base_CmdKeyValues::`vftable'", g_pBase_CmdKeyValues_VFTable);
	...
	virtual void GetCon(void) const
	{
		...
		g_pSVC_ServerTick_VFTable = g_GameDll.GetVirtualMethodTable(".?AVSVC_ServerTick@@");
		g_pSVC_VoiceData_VFTable = g_GameDll.GetVirtualMethodTable(".?AVSVC_VoiceData@@");
		g_pSVC_DurangoVoiceData_VFTable = g_GameDll.GetVirtualMethodTable(".?AVSVC_DurangoVoiceData@@");
		g_pSVC_PlaylistOverrides_VFTable = g_GameDll.GetVirtualMethodTable(".?AVSVC_PlaylistOverrides@@");
		g_pCLC_ClientTick_VFTable = g_GameDll.GetVirtualMethodTable(".?AVCLC_ClientTick@@");
		g_pCLC_SetPlaylistVarOverride_VFTable = g_GameDll.GetVirtualMethodTable(".?AVCLC_SetPlaylistVarOverride@@");
		g_pBase_CmdKeyValues_VFTable = g_GameDll.GetVirtualMethodTable(".?AVBase_CmdKeyValues@@");
