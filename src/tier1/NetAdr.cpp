...
void CNetAdr::ToSockadr(struct sockaddr_in6* const pSadr) const
{
	...
	pSadr->sin6_family = AF_INET6;
	pSadr->sin6_port = port;
	...
	{
		pSadr->sin6_addr = in6addr_loopback;
	}
}
...
bool CNetAdr::SetFromString(const char* const pch, const bool bUseDNS)
{
	...
	{
		// Unbracketed 'host:port' (e.g. 127.0.0.1:37015 or localhost:37015).
		// Only strip when there is exactly ONE colon so bare IPv6 is untouched.
		// Pre-fix: elev cl_rcon_address 127.0.0.1:37015 failed SetFromString
		// because the port stayed on the host and inet_pton rejected it.
		char* const pchColon = strrchr(pszAddress, ':');
		if (pchColon && strchr(pszAddress, ':') == pchColon)
		{
			*pchColon = '\0';
			SetPort(uint16_t(htons(uint16_t(atoi(pchColon + 1)))));
		}
...
bool CNetAdr::SetFromString(const char* const pch, const bool bUseDNS)
{
	...
	{
		// Unbracketed 'host:port' (e.g. 127.0.0.1:37015). Exactly one colon
		// so bare IPv6 addresses are not mis-split.
		char* const pchColon = strrchr(pszAddress, ':');
		if (pchColon && strchr(pszAddress, ':') == pchColon)
		{
			*pchColon = '\0';
