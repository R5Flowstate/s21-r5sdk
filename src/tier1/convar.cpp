...
//-----------------------------------------------------------------------------
// Walk s_pConCommandBases -- ConVar_Register is skipped in the S21 inject path.
//-----------------------------------------------------------------------------
ConCommandBase* Sdk_GetStaticConCommandBaseHead()
{
	return ConCommandBase::s_pConCommandBases;
}
...
ConCommandBase* Sdk_FindStaticConCommandBase(const char* const name)
{
	if (!name)
		return nullptr;
	for (ConCommandBase* cur = ConCommandBase::s_pConCommandBases; cur; cur = cur->m_pNext)
	{
		const char* const n = cur->GetName();
		if (n && V_stricmp(n, name) == 0)
			return cur;
	}
	return nullptr;
}
...
{
	ConCommandBase* const base = Sdk_FindStaticConCommandBase(name);
	if (!base || base->IsCommand())
		return nullptr;
	return static_cast<ConVar*>(base);
}
...
ConCommand* Sdk_FindStaticConCommand(const char* const name)
{
	...
	if (!base || !base->IsCommand())
		return nullptr;
	return static_cast<ConCommand*>(base);
}
...
{
	// S21: no m_pszStaticUsageString. GetUsageText returns
	// whatever SetUsageText has installed at m_pszCustomUsageString (NULL
	// if never called). Matches engine GetUsageText semantics.
	return m_pszCustomUsageString;
}
...
{
	// S21: m_pszStaticUsageString is gone; the ctor now routes the
	// compile-time usage text into m_pszCustomUsageString (see
	// ConCommandBase::Create). So we always read from that single slot.
	return m_pParent->m_pszCustomUsageString;
}
