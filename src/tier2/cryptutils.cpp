	...
	const u8* const aad, const size_t aadSize)
{
	...
	const int setRet = mbedtls_gcm_setkey(&gcm, MBEDTLS_CIPHER_ID_AES, key, ctx.keyBits);
	int cryptRet = MBEDTLS_ERR_ERROR_GENERIC_ERROR;

	if (setRet == 0)
	{
		cryptRet = mbedtls_gcm_crypt_and_tag(&gcm, MBEDTLS_GCM_ENCRYPT, size,
			ctx.iv, sizeof(ctx.iv), aad, aadSize, inBuf, outBuf,
			sizeof(CryptoAuthTag_t), ctx.authTag);
	}

	mbedtls_gcm_free(&gcm);
	return setRet == 0 && cryptRet == 0;
}
	...
	const u8* const aad, const size_t aadSize)
{
	...
	if (setRet == 0)
	{
		cryptRet = mbedtls_gcm_auth_decrypt(&gcm, size, ctx.iv, sizeof(ctx.iv),
			aad, aadSize, ctx.authTag, sizeof(CryptoAuthTag_t), inBuf, outBuf);
	}
