...
{
	return s_bIsDedicated && *s_bIsDedicated;
}
...
void SV_CheckForBanAndDisconnect(CClient* const pClient, const string& svIPAddr, const PlatformUserId_t nUserID, const string& svPersonaName, const int nPort);
...
class HSV_Main : public IDetour
{
	...
	virtual void Detour(const bool bAttach) const
	{
		//DetourSetup(&v_SV_InitGameDLL, SV_InitGameDLL, bAttach);
		//DetourSetup(&v_SV_ShutdownGameDLL, SV_ShutdownGameDLL, bAttach);
		// [IB-DUMP-DEDI] hooked to one-shot dump the instance-baseline blobs
		// after server activation (canyonlands_hu CDynamicProp baseline desync).
		DetourSetup(&v_SV_ActivateServer, SV_ActivateServer, bAttach);
