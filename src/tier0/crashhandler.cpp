...
//-----------------------------------------------------------------------------
// Purpose: [addr, addr+size) is committed and readable. VEH runs before frame __try.
//-----------------------------------------------------------------------------
static bool CrashSafeReadable(uintptr_t addr, size_t size)
{
	if (!addr)
		return false;
...
void CCrashHandler::FormatRegisters()
{
	...
	// GP regs that look like C++ objects: emit vtable and slot 7 (+0x38).
	m_Buffer.Append("objprobe:\n{\n");
	{
		const struct { const char* name; DWORD64 val; } probeRegs[] = {
			{ "rcx", pContextRecord->Rcx }, { "rbx", pContextRecord->Rbx },
			{ "rdx", pContextRecord->Rdx }, { "rsi", pContextRecord->Rsi },
			{ "rdi", pContextRecord->Rdi }, { "r8 ", pContextRecord->R8  },
			{ "r9 ", pContextRecord->R9  }, { "r10", pContextRecord->R10 },
			{ "r11", pContextRecord->R11 }, { "r12", pContextRecord->R12 },
			{ "r13", pContextRecord->R13 }, { "r14", pContextRecord->R14 },
			{ "r15", pContextRecord->R15 },
		};

		const size_t probeCount = sizeof(probeRegs) / sizeof(probeRegs[0]);
		for (size_t i = 0; i < probeCount; i++)
		{
			const uintptr_t obj = (uintptr_t)probeRegs[i].val;
			if (!CrashSafeReadable(obj, sizeof(uintptr_t)))
				continue;
			...
			if (VirtualQuery(reinterpret_cast<LPCVOID>(vtbl), &mbi, sizeof(mbi)) == 0)
				continue;
			if (mbi.Type != MEM_IMAGE) // vtables live in a module's.rdata
				continue;

			if (CrashSafeReadable(vtbl + 0x38, sizeof(uintptr_t)))
			{
				const uintptr_t slot7 = *reinterpret_cast<uintptr_t*>(vtbl + 0x38);
				m_Buffer.AppendFormat("\t%-3s=%p vtable=%p slot7(+0x38)=%p\n",
					probeRegs[i].name, (void*)obj, (void*)vtbl, (void*)slot7);
			}
			else
			{
				m_Buffer.AppendFormat("\t%-3s=%p vtable=%p (slot7 unreadable)\n",
					probeRegs[i].name, (void*)obj, (void*)vtbl);
			}
		}
	}
	m_Buffer.Append("}\n");
...
// When set, dump and continue; do not ExitProcess. Unhandled exceptions still die via UEF.
bool g_bCrashHandlerNoFatal = false;
...
long __stdcall BottomLevelExceptionFilter(EXCEPTION_POINTERS* const pExceptionInfo)
{
	...
	{
		if (g_bCrashHandlerNoFatal)
		{
			// Re-arm so the next crash is dumped too, then hand the exception
			// to the SEH chain (an SDK __except wrapper recovers it).
			g_CrashHandler.SetExit(false);
			g_CrashHandler.End();
			return EXCEPTION_CONTINUE_SEARCH;
		}
		// The file sinks are async: drain before the process dies, or the lines
		// that explain this crash never reach disk.
		if (g_pfnPlatLogDrain)
			g_pfnPlatLogDrain();
