...
inline int CBaseHandle::GetEntryIndex() const
{
	...
	//
	// Valve + dedi half: entry bits only. Without ENT_ENTRY_MASK, callers that
	// index entity tables with GetEntryIndex() treat entry|serial as a raw index
	// (OOB / GetChildren class).
	if ( !IsValid() )
...
inline int CBaseHandle::GetEntryIndex() const
{
	...
	// retains the prior (bug-submarining) behavior.
	//
	// Keep ENT_ENTRY_MASK on the valid path: without it GetEntryIndex returns
	// full m_Index (entry|serial) and LookupEntity OOB-indexes past
	// NUM_ENT_ENTRIES; serial checks are skipped and stale slots can resolve.
	if ( !IsValid() )
		return NUM_ENT_ENTRIES-1;
	return m_Index & ENT_ENTRY_MASK;
}
