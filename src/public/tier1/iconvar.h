...
// These size the char[MAXITEMS][ITEM_LENGTH] completion array. CConsole::AutoComplete routes ALL
// console autocompletion through OUR ConCommand::AutoCompleteSuggest (a non-virtual qualified call,
// see IConsole.cpp) so this array is always allocated HERE by SDK code with these dims -- we never
// hit the ENGINE's virtual AutoCompleteSuggest with its own (smaller) array. That engine path is
// what crashed `give`: its completion (195 weapon.nut files) wrote past the engine's array and
// smashed the heap. Bypassing it removes any dependency on the engine's array size, so we keep
// these generous enough to hold our largest completion (give) plus any native completion's output.
