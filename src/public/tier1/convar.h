...
//private
	...
	// S21: one usage-text field at +0x28; sizeof(ConCommandBase)=0x38.
	const char*              m_pszCustomUsageString; //0x0028
...
static_assert(sizeof(ConCommandBase) == 0x38);
...
protected:
	...
	float          m_fMinVal;         //0x006C
	bool           m_bHasMax;         //0x0070
	float          m_fMaxVal;         //0x0074
	CUtlVector<CVChange_t> m_fnChangeCallbacks; //0x0078
};
static_assert(sizeof(ConVar) == 0x98);
...
//-----------------------------------------------------------------------------
// Static ConVar walk helpers. Declared outside the dual CONVAR_H halves (PCH trap).
//-----------------------------------------------------------------------------
#if defined(CLIENT_DLL) && !defined(SDK_STATIC_CONVAR_HELPERS_H)
#define SDK_STATIC_CONVAR_HELPERS_H
...
#endif // CLIENT_DLL && !SDK_STATIC_CONVAR_HELPERS_H
