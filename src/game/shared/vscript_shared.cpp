//=============================================================================//
//
// Purpose: expose native code to the shared VScript API.
//
// Bindings are static SQRESULT functions taking a single HSQUIRRELVM, returning
// through SCRIPT_CHECK_AND_RETURN and registering via
// DEFINE_SHARED_SCRIPTFUNC_NAMED with a "SharedScript_" prefix.
// This TU also detours the enum registrars and null-guards SQObject_ToString
// during remote-function registration (srcObj/dstObj can be NULL).
//
//=============================================================================//
...
//-----------------------------------------------------------------------------
// Purpose: player-facing product label (HUD / netgraph). Not the wire gate.
//-----------------------------------------------------------------------------
static SQRESULT SharedScript_GetSDKDisplayVersion(HSQUIRRELVM v)
{
    sq_pushstring(v, SDK_DISPLAY_VERSION, sizeof(SDK_DISPLAY_VERSION) - 1);
    SCRIPT_CHECK_AND_RETURN(v, SQ_OK);
}
...
{
	const DataTableHeader* header = Datatable_ResolveFromScript(v);
	if (!header)
		return SQ_ERROR;

	sq_pushinteger(v, header->rowCount);
	return SQ_OK;
}
...
void Script_RegisterCommonAbstractions(CSquirrelVM* s)
{
    ...
    DEFINE_SHARED_SCRIPTFUNC_NAMED(s, GetSDKDisplayVersion, "Gets the player-facing product version", "string", "", false);
