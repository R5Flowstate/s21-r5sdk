...
public:
	...
	inline float           Diag_Friction(void)    const { return m_flFriction; }
	inline const Vector3D& Diag_BaseVel(void)     const { return m_vecBaseVelocity; }

	// Collision-watch accessors ([COLL-WATCH]). The trace hull (OBB collision
	// bounds the movement hull-sweep uses) and the ground entity the player is
	// standing on. m_hGroundEntity is the authoritative "what am I resting on"
	// handle; -1 == not resting on any entity (airborne, or on the world brush).
	inline const Vector3D& Diag_CollMins(void)    const { return m_Collision.Diag_Mins(); }
	inline const Vector3D& Diag_CollMaxs(void)    const { return m_Collision.Diag_Maxs(); }
	inline int             Diag_GroundEntIndex(void) const { return m_hGroundEntity.IsValid() ? m_hGroundEntity.GetEntryIndex() : -1; }
...
protected:
	...
	int m_entIndex;
	char gap_74[4];
	string_t m_iClassname; // NOT a pointer -- matches every other string_t field in this
	                       // class (m_ModelName, m_iName, m_target,...); verified
	                       // after a crash proved the extra indirection reads
	                       // garbage (string_t is an 8-byte struct wrapping one const char*,
	                       // same size as a pointer, so this fix does not shift any
