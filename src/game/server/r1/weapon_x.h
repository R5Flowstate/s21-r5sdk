...
public:
	...
	// Raw m_modVars access for KV-schema-parsed weapon fields that don't yet have a
	// named accessor (see internal notes for the
	// full m_modVars byte-offset table this indexes into, e.g. viewkick_* fields).
	inline const char* GetModVarsRaw() const { return m_modVars; }
	inline const EHANDLE& GetWeaponOwnerHandle() const { return m_weaponOwner; }
