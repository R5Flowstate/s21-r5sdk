...
public:
	...
	{
		m_lungeTargetEntity.Term();
		m_isLungingToPosition = false;
		m_lungeTargetPosition.Init();
		m_lungeStartPositionOffset.Init();
		m_lungeStartPositionOffset_notLagCompensated.Init();
		m_lungeEndPositionOffset.Init();
		m_lungeStartTime = 0.0f;
		m_lungeEndTime = 0.0f;
		m_lungeCanFly = false;
		m_lungeLockPitch = false;
		m_lungeStartPitch = 0.0f;
		m_lungeSmoothTime = 0.0f;
		m_lungeMaxTime = 0.0f;
		m_lungeMaxEndSpeed = 0.0f;
		m_melee.lungeBoost.Init();
	}

	bool HasLungeTargetEntity(void) const { return m_lungeTargetEntity.IsValid(); }
	uint32_t GetLungeTargetEntityHandle(void) const
	{
		return static_cast<uint32_t>(m_lungeTargetEntity.ToInt());
	}
private:
	...
	char gap_5ee6[2];
	PlatformUserId_t m_platformUserId;
	char m_hardware;
	char gap_5ef1[7];
...
public:
	void RunNullCommand(void);
	QAngle* EyeAngles(QAngle* pAngles);
	Vector3D* EyePosition(Vector3D* pPosition);

	void SetTimeBase(float flTimeBase);
	float GetTimeBase() const;

	void SetLastUCmdSimulationTicks(int nTicks);
	void SetLastUCmdSimulationRemainderTime(float fRemainderTime);
	void SetTotalExtraClientCmdTimeAttempted(float flAttemptedTime);
	...
	// exposes the punch
	// spring fields for the dedi-side view-kick impulse fix.
	inline CurrentData_LocalPlayer& GetCurrentFrameLocalPlayer() { return m_currentFrameLocalPlayer; }
	...
	inline int Diag_DuckState() const { return m_duckState; }  // pose/stance index ([MOVE-DIAG])

	// Button mask from the last usercmd the server ran for this player.
	inline int Diag_Buttons() const { return m_nButtons; }

	// Collision-watch accessors ([COLL-WATCH]). The per-pose hull cache the engine
	// fills in UpdatePlayerClassDependentVars from the player's
	// settings block via the poseSettings finder globals -- a degenerate (min==max)
	// or NaN hull here is the PatchPlayerLayoutGlobals failure cause. m_StuckLast
	// hull started a move embedded in solid = stuck-in-geometry).
	inline const Vector3D& Diag_StandHullMin() const { return m_StandHullMin; }  // CPlayer+0x65FC
	inline const Vector3D& Diag_StandHullMax() const { return m_StandHullMax; }  // CPlayer+0x6608
	inline const Vector3D& Diag_DuckHullMin()  const { return m_DuckHullMin; }   // CPlayer+0x6614
	inline const Vector3D& Diag_DuckHullMax()  const { return m_DuckHullMax; }   // CPlayer+0x6620
	inline int             Diag_StuckLast()    const { return m_StuckLast; }     // CPlayer+0x5A88
	inline bool            Diag_NeedStuckCheck() const { return m_needStuckCheck; }

	// [SIMTICK] noclip-jitter cadence diagnostic. m_lastUCmdSimulationTicks is the
	// player's tick base advanced inside PhysicsSimulate; its per-frame delta = the
	// number of usercmd ticks the dedi replayed this server frame (the 1-2-3 batch).
	inline int             Diag_LastSimTicks() const { return m_lastUCmdSimulationTicks; }
	...
	// read back only as (curTime - it), so the drain slack has to be undone here too.
	inline void SetLastSimulateTime(float flCurTime) { m_lastSimulateTime = flCurTime; }

	// [MANTLE-BOOST] traversal readers for the mantle-exit boost bridge
	// (mantle_boost_bridge.cpp). Feed the S21 anim-camera trigger's ledge basis.
	inline float Diag_TraversalProgress(void)  const { return m_traversalProgress; }
	inline float Diag_TraversalAnimProgress(void) const { return m_currentFramePlayer.m_traversalAnimProgress; }
	inline float Diag_TraversalStartTime(void) const { return m_traversalStartTime; }
	inline const Vector3D& Diag_TraversalForwardDir(void) const { return m_traversalForwardDir; }
	inline const Vector3D& Diag_TraversalRefPos(void)     const { return m_traversalRefPos; }

	// [MANTLE-BOOST] state-4 slide-chain grant, split in engine order: the
	// m_lastSlideTime stamp lands on the velocity arm BEFORE the forced Jump,
	// the three flags after it (so Jump's height ladder does not read m_sliding).
	// m_lastSlideBoost is NOT one of them -- it is the slide speed GAIN in units,
	// which Jump subtracts back out on a slide jump inside 0.4s; a timestamp there
	// zeroes the next jump's velocity.
	inline void MantleBoost_StampSlideTime(float flCurTime)
	{
		m_lastSlideTime = flCurTime;
	}
	inline void MantleBoost_GrantSlide(void)
	{
		m_sliding = true;
...
// the per-pose hull cache to entity+26108(0x65FC)..+26152(0x6628)).
static_assert(sizeof(CPlayer) == 0x7EF0);

inline QAngle*(*CPlayer__EyeAngles)(CPlayer* pPlayer, QAngle* pAngles);
inline Vector3D*(*CPlayer__EyePosition)(CPlayer* pPlayer, Vector3D* pPosition);
inline void(*CPlayer__PlayerRunCommand)(CPlayer* pPlayer, CUserCmd* pUserCmd, IMoveHelper* pMover);
inline bool(*CPlayer__PhysicsSimulate)(CPlayer* pPlayer, int numPerIteration, bool adjustTimeBase);
inline void(*CPlayer__ApplyViewPunch)(CPlayer* pPlayer, const CTakeDamageInfo* inputInfo);
...
class VPlayer : public IDetour
{
	...
	{
		LogFunAdr("CPlayer::EyeAngles", CPlayer__EyeAngles);
		LogFunAdr("CPlayer::EyePosition", CPlayer__EyePosition);
		LogFunAdr("CPlayer::PlayerRunCommand", CPlayer__PlayerRunCommand);
		LogFunAdr("CPlayer::PhysicsSimulate", CPlayer__PhysicsSimulate);
		LogFunAdr("CPlayer::ApplyViewPunch", CPlayer__ApplyViewPunch);
	}
	...
	{
		Module_FindPattern(g_GameDll, "40 53 48 83 EC 30 F2 0F 10 05 ?? ?? ?? ??").GetPtr(CPlayer__EyeAngles);
		CMemory(CPlayer__EyeAngles).FindPatternSelf("40 53 48 83 EC 20 41 B0 01 48 8B DA E8", CMemory::Direction::DOWN, 0x3000).GetPtr(CPlayer__EyePosition);
		Module_FindPattern(g_GameDll, "E8 ?? ?? ?? ?? 8B 03 49 81 C6 ?? ?? ?? ??").FollowNearCallSelf().GetPtr(CPlayer__PlayerRunCommand);
		Module_FindPattern(g_GameDll, "E8 ?? ?? ?? ?? 48 8B 15 ?? ?? ?? ?? 84 C0 74 06").FollowNearCallSelf().GetPtr(CPlayer__PhysicsSimulate);
		Module_FindPattern(g_GameDll, "4C 8B DC 49 89 5B ?? 49 89 6B ?? 49 89 7B ?? 41 54 41 56 41 57 48 81 EC").GetPtr(CPlayer__ApplyViewPunch);
	}
