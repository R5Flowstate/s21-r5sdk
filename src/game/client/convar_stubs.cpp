...
static ConVar hud_setting_healthbar_style("hud_setting_healthbar_style", "0", FCVAR_RELEASE | FCVAR_ARCHIVE,
...
static ConVar serverbrowser_gameModeFilter("serverbrowser_gameModeFilter", "0", FCVAR_RELEASE,
	"Server browser playlist filter dialog-list index (0 = Any).");
static ConVar serverbrowser_hideEmptyServers("serverbrowser_hideEmptyServers", "0", FCVAR_RELEASE,
	"Hide empty (0-player) servers in the server browser (0 = No, 1 = Yes).");


// EULA / first-time-seen (engine + scripts read these; ARCHIVE so accept sticks across runs).
// Current bridge terms version -- bump when BRIDGE_EULA_BODY text changes and re-prompt.
static ConVar eula_version("eula_version", "4", FCVAR_RELEASE | FCVAR_ARCHIVE,
	"Current bridge EULA / terms version (GetCurrentEULAVersion).");
static ConVar eula_version_accepted("eula_version_accepted", "0", FCVAR_RELEASE | FCVAR_ARCHIVE,
	"Last accepted EULA version (0 = never accepted / first-time). SetEULAVersionAccepted writes this.");
static ConVar r5f_display_version("r5f_display_version", SDK_DISPLAY_VERSION, FCVAR_RELEASE,
	"Player-facing product version shown in HUD.");
static ConVar first_time_player_state("first_time_player_state", "0", FCVAR_RELEASE | FCVAR_ARCHIVE,
	"First-time player state machine (GetFirstTimePlayerState / SetFirstTimePlayerState).");
static ConVar firsttime_mp_message("firsttime_mp_message", "0", FCVAR_RELEASE | FCVAR_ARCHIVE,
	"First-time multiplayer message seen flag.");
...
// Stub for scripts (sh_survival_commentary ShouldPlayBattleChatter); dedi engine never shipped this cvar.
static ConVar sound_spectator_dialogue("sound_spectator_dialogue", "1", FCVAR_RELEASE | FCVAR_ARCHIVE,
	"Play battle chatter / dialogue for dead/spectating listeners.");
...
static ConVar voice_input_device("voice_input_device", "", FCVAR_RELEASE | FCVAR_ARCHIVE, "Voice input device name.");
// Stub for scripts (sh_survival_commentary ShouldPlayBattleChatter); dedi engine never shipped this cvar.
// Without this stub: SCRIPT ERROR "ConVar sound_spectator_dialogue is not valid".
static ConVar sound_spectator_dialogue("sound_spectator_dialogue", "1", FCVAR_RELEASE | FCVAR_ARCHIVE,
	"Play battle chatter / dialogue for dead/spectating listeners.");
