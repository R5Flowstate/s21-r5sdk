...
// ConVars
//-----------------------------------------------------------------------------
static void RuiDrawEnableChanged_f(IConVar* var, const char* pOldValue, float flOldValue, ChangeUserData_t pUserData);
...
static void RuiDrawEnableChanged_f(IConVar* var, const char* pOldValue, float flOldValue, ChangeUserData_t pUserData)
{
	...
	const int nValue = rui_drawEnable.GetBool() ? 1 : 0;
	if (ConVar* const pHud = g_pCVar->FindVar("cl_drawhud"))
		pHud->SetValue(nValue);
	if (ConVar* const pText = g_pCVar->FindVar("rui_text_drawing_enabled"))
		pText->SetValue(nValue);

	Msg(eDLL_T::RTECH, "[RUI] rui_drawEnable %d\n", nValue);
}
...
void Rui_BindDrawEnable(void)
{
	...
	rui_drawEnable.InstallChangeCallback(RuiDrawEnableChanged_f, nullptr, false);
...
void VRuiDrawEnable::GetFun(void) const
{
	// S21 UI_DrawStageInstanceRange. Frame size is wildcarded: DX11 0xE8, DX12 0xF8.
	Module_FindPattern(g_GameDll,
		"48 8B C4 55 41 56 48 81 EC ?? ?? 00 00 80 3D ?? ?? ?? ?? 00 "
		"48 89 58 ?? 48 89 70 ?? 48 89 78 ?? 41 8B F8")
		.GetPtr(v_UI_DrawStageInstanceRange);
	if (!v_UI_DrawStageInstanceRange)
		Warning(eDLL_T::RTECH, "[RUI] UI_DrawStageInstanceRange pattern unresolved\n");
}

void VRuiDrawEnable::Detour(const bool bAttach) const
{
	if (v_UI_DrawStageInstanceRange)
		DetourSetup(&v_UI_DrawStageInstanceRange, &Hook_UI_DrawStageInstanceRange, bAttach);
}
...
// DrawImage: ui.dll blur sentinel -4 -> 0x2000. Reject offsets past RUI_MAX_DATASTRUCT_SIZE.
...
//=============================================================================
// SetArgByName: null-check definition pointer (RVA 0x301869).
//=============================================================================
...
//=============================================================================
// UpdateWidget: null-check definition pointer (RVA 0x2FC685).
//=============================================================================
...
//=============================================================================
// ProcessAllWidgets: null instance check (RVA 0x2FD383).
//=============================================================================
...
//=============================================================================
// ProcessAllWidgets: definition pointer check (RVA 0x2FD3CB).
//=============================================================================
...
//=============================================================================
// DestroyWidget: null stale instance pointers before HeapFree (RVA 0x71CB60).
//=============================================================================
...
//=============================================================================
// After ruiFunc, revalidate the definition (RVA 0x2FC830).
//=============================================================================
...
//=============================================================================
// VtableFunc19 GetTransformSize null check (RVA 0x2FBDA0).
//=============================================================================
...
//=============================================================================
// VtableFunc20 GetRenderJobDimensions null check (RVA 0x2FBDD0).
//=============================================================================
...
//=============================================================================
// TILING mode: NOP UV/rotation overrides; keep identity transform.
//=============================================================================
...
static void Rui_RemapStateS3toS16(uint8_t* shadow, const uint8_t* s)
{
	...
	wr32(0xB4, rd32(0xB0));  // hasOpenMenu
	wr32(0xB8, rd32(0xB4));  // isPhaseShifted
	wr32(0xBC, rd32(0xB8));  // isOneHanded
	wr32(0xC0, rd32(0xBC));  // adsFrac (float)
	...
	// nxMode and padding at +0x108..+0x10F -- zero
	wr32(0x108, 0);
	wr32(0x10C, 0);
}
