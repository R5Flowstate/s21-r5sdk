...
public:
    ...
    void ToggleTab(const OverlayTab_e tab);
    OverlayTab_e GetCurrentTab(void) const { return m_currentTab; }

private:
    // Forward declaration for the recursive struct
    struct PresetCommand_t;

    void DrawConsolePanel(void);
    ...
    bool CollectSuggestionsFromRegistry(void);
...
private:
    ...
    // History-buffer index; empty input means the arrow keys walk submitted commands.
    int                            m_historyPos;
    ...
    // Set when the input text has been modified so suggestions rebuild.
    ...
    // If set, up/down arrows walk autocomplete instead of command history.
    ...
    OverlayTab_e                   m_currentTab;
    int                            m_selectTab; // OverlayTab_e, or -1 if none
